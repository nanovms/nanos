...
static boolean basic_test(heap h)
{
	...
	heap id = create_id_heap(h, h, base, length, pagesize);
...
static boolean random_test(heap h, heap rh, u64 page_order, int churn)
{
    ...
    heap id = create_id_heap_backed(h, h, rh, pagesize);
...
{
    heap idh = create_id_heap(h, h, 0, GTE_TEST_MAX, 1);
...
static boolean alloc_subrange_test(heap h)
{
    ...
    heap id = create_id_heap(h, h, SUBRANGE_TEST_MIN, SUBRANGE_TEST_LENGTH, PAGESIZE);
