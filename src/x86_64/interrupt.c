...
static inline void write_tss_u64(struct cpuinfo_machine *cim, int offset, u64 val)
{
    u64 * vec = (u64 *)(u64_from_pointer(&cim->tss) + offset);
    *vec = val;
}

void set_ist(struct cpuinfo_machine *cim, int i, u64 sp)
{
    assert(i > 0 && i <= 7);
    write_tss_u64(cim, 0x24 + (i - 1) * 8, sp);
}
