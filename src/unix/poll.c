...
{
#ifdef __x86_64__
    register_syscall_nolock(map, epoll_create, epoll_create);
    register_syscall_nolock(map, epoll_wait, epoll_wait);
    register_syscall_nolock(map, poll, poll);
    register_syscall_nolock(map, select, select);
#endif
    register_syscall_nolock(map, epoll_create1, epoll_create);
    register_syscall_nolock(map, epoll_ctl, epoll_ctl);
    register_syscall_nolock(map, ppoll, ppoll);
    register_syscall_nolock(map, pselect6, pselect);
    register_syscall_nolock(map, epoll_pwait, epoll_wait); /* sigmask unused right now */
}
