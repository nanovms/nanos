...
closure_function(1, 0, void, intx_handler,
                 pvscsi, dev)
{
    ...
    if ((status & PVSCSI_INTR_ALL_SUPPORTED) == 0)
        return;
    /* clear pending */
    pvscsi_reg_write(dev, PVSCSI_REG_OFFSET_INTR_STATUS, status);
...
static void pvscsi_attach(heap general, storage_attach a, heap page_allocator, pci_dev d)
{
    ...
    boolean msix_enabled = pci_enable_msix(dev->dev) != 0;
...
void init_pvscsi(kernel_heaps kh, storage_attach a)
{
    ...
    register_pci_driver(closure(h, pvscsi_probe, h, a, (heap)heap_linear_backed(kh)), 0);
}
